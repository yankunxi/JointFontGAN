...
def main():
    ...
    for font in opt.font:
        ...
        gt_sk_zipfile = ZipFile(gt_sk_file, "r")
        gt_sk_list = []
        for j in folder2gt_sk1():
            temp_img = Image.open(gt_sk_zipfile.open(font + '_' + j
                                                     + '.png'))
            temp_img = ImageEnhance.Contrast(temp_img).enhance(4)
            temp_img = ImageEnhance.Brightness(
                temp_img).enhance(1.1)
            temp_img = ImageEnhance.Sharpness(temp_img).enhance(2)
            gt_sk_list += [temp_img]
        # gt_sk_list = [Image.open(gt_sk_zipfile.open(font + '_' + j
        #                                             + '.png')) for
        #               j in folder2gt_sk1()]
        img_sks += [redbox_gt(gt_sk_list[0], gt_sk_list[1], ch)]
