...
class StackGANModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.output_A0 = self.Tensor(opt.batchSize, opt.input_nc, 
                                     opt.fineSize, opt.fineSize * 3)
        self.output_B0 = self.Tensor(opt.batchSize, opt.output_nc,
                                     opt.fineSize, opt.fineSize)
        ...
        self.netE1 = XInetworks.define_Enc(opt.input_nc_1,
                                           opt.output_nc_1, opt.ngf,
                                           opt.which_model_netG,
                                           opt.norm, opt.use_dropout1,
                                           self.gpu_ids)

        self.netDE1 = XInetworks.define_Dec(opt.input_nc_1,
                                            opt.output_nc_1, opt.ngf,
                                            opt.which_model_netG,
                                            opt.norm,
                                            opt.use_dropout1,
                                            self.gpu_ids)
        disc_ch_1 = opt.input_nc_1
        ...
        if self.opt.conditional:
            ...
            nif = disc_ch_1 + disc_ch_1
            netD_norm = opt.norm
            self.netD1 = XInetworks.define_D(nif, opt.ndf,
                                             opt.which_model_netD,
                                             opt.n_layers_D,
                                             netD_norm, use_sigmoid,
                                             True, self.gpu_ids)
        latest = opt.continue_latest
        if self.isTrain:
            if self.opt.conv3d:
                self.load_network(self.netG_3d, 'G_3d',
                                   latest=latest, epoch=self.epoch_str1)
            self.load_network(self.netG, 'G', latest=latest, epoch=self.epoch_str1)

            if opt.which_epoch>0:
                print("Load StyleNet from its pretrained model...")
                self.load_network(self.netE1, 'E1', latest=latest, epoch=self.epoch_str1)
                self.load_network(self.netDE1, 'DE1',
                                  latest=latest, epoch=self.epoch_str1)
                self.load_network(self.netD1, 'D1', latest=latest, epoch=self.epoch_str1)
                if opt.which_model_preNet != 'none':
        ...
        if not self.isTrain:
            ...
            if opt.no_Style2Glyph:
                if self.opt.conv3d:
                    self.load_network(self.netG_3d, 'G_3d',
                                      latest=latest, epoch=self.opt.which_epoch1)
                self.load_network(self.netG, 'G', latest=latest,
                                  epoch=self.opt.which_epoch1)
                self.load_network(self.netE1, 'E1', latest=latest, epoch=self.epoch_str1)
                self.load_network(self.netDE1, 'DE1',
                                  latest=latest, epoch=self.epoch_str1)
                self.load_network(self.netD1, 'D1', latest=latest, epoch=self.epoch_str1)
                if opt.which_model_preNet != 'none':
                    self.load_network(self.preNet_A, 'PRE_A', latest=latest, epoch=self.epoch_str1)
            else:
                if self.opt.conv3d:
                    self.load_network(self.netG_3d, 'G_3d', latest=latest, epoch=self.epoch_str1)
                self.load_network(self.netG, 'G', latest=latest, epoch=self.epoch_str1)
                self.load_network(self.netE1, 'E1', latest=latest, epoch=self.epoch_str1)
                self.load_network(self.netDE1, 'DE1', latest=latest, epoch=self.epoch_str1)
                self.load_network(self.netD1, 'D1', latest=latest, epoch=self.epoch_str1)
                if opt.which_model_preNet != 'none':
                    self.load_network(self.preNet_A, 'PRE_A', latest=latest, epoch=self.epoch_str1)
        ...
        if self.isTrain:
            ...
            if opt.diff_loss1 == 'L1':
                self.criterionDiff1 = torch.nn.L1Loss()
            elif opt.diff_loss1 == 'MSE':
                self.criterionDiff1 = torch.nn.MSELoss()
            ...
            self.optimizer_DE1 = torch.optim.Adam(
                self.netDE1.parameters(),
                lr=opt.lr, betas=(opt.beta1, 0.999))
    ...
    def set_input(self, input):
        ...
        if self.opt.base_font:
            ...
            self.input_base.resize_(input_base.size()).copy_(
                input_base)

            b, c, m, n = self.input_base.size()

            real_base = self.Tensor(self.opt.output_nc,
                                    self.opt.input_nc_1, m, n)
            for batch in range(self.opt.output_nc):
                if not self.opt.rgb_in and self.opt.rgb_out:
                    real_base[batch, 0, :, :] = self.input_base[0,
                                                batch, :, :]
                    real_base[batch, 1, :, :] = self.input_base[0,
                                                batch, :, :]
                    real_base[batch, 2, :, :] = self.input_base[0,
                                                batch, :, :]
        ...
        if self.opt.isTrain:
            ...
            for i, im in enumerate(self.image_paths):
                self.id_[int(
                    im.split('/')[-1].split('.png')[0].split('_')[
                        -1])] = i
                self.obs += [int(
                    im.split('/')[-1].split('.png')[0].split('_')[
                        -1])]
            for i in list(
                    set(range(self.opt.output_nc)) - set(self.obs)):
                self.id_[i] = np.random.randint(low=0, high=len(
                    self.image_paths))
    ...
    def all2observed(self, tensor_all):
        ...
        for batch in range(b):
            if not self.opt.rgb_in and self.opt.rgb_out:
                tensor_gt[batch, 0, :, :] = tensor_all.data[batch,
                                            self.out_id[batch], :, :]
                tensor_gt[batch, 1, :, :] = tensor_all.data[batch,
                                            self.out_id[batch], :, :]
                tensor_gt[batch, 2, :, :] = tensor_all.data[batch,
                                            self.out_id[batch], :, :]
            else:
                # TODO
                tensor_gt[batch, :, :, :] = tensor_all.data[batch,
                                            self.out_id[
                                                batch] * np.array(
                                                self.opt.input_nc_1):(
                                                                                 self.out_id[
    ...
    def forward0(self):
        self.real_AA = self.input_A0.detach()
        if self.opt.conv3d:
            self.real_A0_indep = self.netG_3d.forward(
                self.real_AA.unsqueeze(2))
            self.fake_BB = self.netG.forward(
                self.real_A0_indep.squeeze(2))
        else:
            self.fake_BB = self.netG.forward(self.real_AA)
        b, c, m, n = self.fake_BB.size()
        self.real_A0 = self.real_AA.narrow(3, m, m)
        self.fake_B0 = self.fake_BB.narrow(3, m, m)
        if self.initial:
            if self.opt.orna:
    ...
    def forward1(self, inp_grad=False):
        ...
        self.batch_ = b
        self.out_id = self.obs
        real_A1 = self.Tensor(self.opt.output_nc, self.opt.input_nc_1,
                              m, n)
        if self.opt.orna:
        ...
        for batch in range(self.opt.output_nc):
            if not self.opt.rgb_in and self.opt.rgb_out:
                real_A1[batch, 0, :, :] = inp_orna.data[
                                          self.id_[batch], batch, :,
                                          :]
                real_A1[batch, 1, :, :] = inp_orna.data[
                                          self.id_[batch], batch, :,
                                          :]
                real_A1[batch, 2, :, :] = inp_orna.data[
                                          self.id_[batch], batch, :,
                                          :]
            else:
                # TODO
                real_A1[batch, :, :, :] = inp_orna.data[batch,
                                          self.out_id[
                                              batch] * np.array(
                                              self.opt.input_nc_1):(
                                                                               self.out_id[
        ...
        self.real_A1_gt_s = Variable(self.all2observed(inp_orna),
                                     requires_grad=True)
        ...
        if self.opt.base_font:
            ...
            self.real_base_gt = (
                Variable(real_base_gt.data, requires_grad=False))
    ...
    def test(self):
        ...
        if self.opt.conv3d:
            ...
            self.fake_B0 = self.netG.forward(
                self.real_A0_indep.squeeze(2))
        else:
            self.fake_B0 = self.netG.forward(self.real_A0)
        b, c, m, n = self.fake_B0.size()
        self.real_AA = self.real_A0.narrow(3, m, m)
        self.fake_BB = self.fake_B0.narrow(3, m, m)
        ...
        real_A1 = self.Tensor(self.batch_, self.opt.input_nc_1, m, m)

        if self.opt.orna:
            inp_orna = self.real_AA
        else:
            inp_orna = self.fake_BB
        ...
        self.real_B1 = self.input_B0.detach()
    ...
    def prepare_data(self):
        if self.opt.conditional:
            if self.opt.base_font:
                ...
                self.first_pair = Variable(self.real_A1.data,
                                           requires_grad=False)
                self.first_pair_gt = Variable(self.real_A1_gt.data,
                                              requires_grad=False)
    ...
    def backward_D1(self):
        ...
        if self.opt.conditional:
            ...
            if self.opt.which_model_preNet != 'none':
                # transform the input
                transformed_AB1 = self.preNet_A.forward(
                    fake_AB1.detach())
                self.pred_fake_GL = self.netD1.forward(
                    transformed_AB1)

            self.loss_D1_fake = 0
            self.loss_D1_fake += self.criterionGAN(self.pred_fake1,
                                                   label_fake)

            if self.opt.which_model_preNet != 'none':
                self.loss_D1_fake += self.criterionGAN(
                    self.pred_fake_GL, label_fake)
        ...
        if self.opt.conditional:
            ...
            if self.opt.which_model_preNet != 'none':
                ...
                self.pred_real1_GL = self.netD1.forward(
                    transformed_real_AB1)

            self.loss_D1_real = 0
            self.loss_D1_real += self.criterionGAN(self.pred_real1,
                                                   label_real)
            if self.opt.which_model_preNet != 'none':
                self.loss_D1_real += self.criterionGAN(
                    self.pred_real1_GL, label_real)
    ...
    def backward_G(self, pass_grad, iter):
        ...
        if not self.opt.lambda_C or (iter > 700):
            ...
            weights = torch.ones(b, c, m,
                ...
                b, c, m, n)
            obs_ = torch.cuda.LongTensor(
                self.obs) if self.opt.gpu_ids else LongTensor(
                self.obs)
            weights.index_fill_(1, obs_, weight_val)
            weights = Variable(weights, requires_grad=False)

            self.loss_G_L1 = self.criterionDiff0(weights * self.fake_B0,
                                              weights * self.fake_B0_init.detach()) * self.opt.lambda_C
    ...
    def backward_G1(self, iter):
        ...
        if self.opt.conditional:
            ...
            fake_AB = torch.cat(
                (self.first_pair.detach(), self.fake_B1), 1)
            ...
            if self.opt.which_model_preNet != 'none':
                self.loss_G1_GAN += self.criterionGAN(pred_fake_GL,
                                                      True)

        self.loss_G1_L1 = self.criterionDiff0(self.fake_B1_gt,
                                           self.real_B1) * self.opt.lambda_A
        fake_B1_gray = 1 - torch.nn.functional.sigmoid(100 * (
                    torch.mean(self.fake_B1, dim=1,
                               keepdim=True) - 0.9))
        real_A1_gray = 1 - torch.nn.functional.sigmoid(100 * (
                    torch.mean(self.real_A1, dim=1,
                               keepdim=True) - 0.9))
        self.loss_G1_MSE_rgb2gay = self.criterionDiff1(fake_B1_gray,
                                                     real_A1_gray.detach()) * self.opt.lambda_A / 3.0

        real_A1_gt_gray = 1 - torch.nn.functional.sigmoid(100 * (
                    torch.mean(self.real_A1_gt, dim=1,
                               keepdim=True) - 0.9))
        real_B1_gray = 1 - torch.nn.functional.sigmoid(100 * (
                    torch.mean(self.real_B1, dim=1,
                               keepdim=True) - 0.9))

        self.loss_G1_MSE_gt = self.criterionDiff1(real_A1_gt_gray,
                                                real_B1_gray) * self.opt.lambda_A
        ...
        self.real_A1_grad = torch.zeros(b, c, m,
                                        n).cuda() if self.opt.gpu_ids else torch.zeros(
            b, c, m, n)

        if G1_L1_update:
            for batch in self.obs:
                self.real_A1_grad[batch, :, :,
                :] = self.real_A1_gt_s.grad.data[self.id_[batch], :,
                     :, :]
    ...
    def optimize_parameters_Stacked(self, iter):
        ...
        fake_B0_grad = torch.zeros(b, c, m,
                                   n).cuda() if self.opt.gpu_ids else torch.zeros(
            b, c, m, n)
        ...
        for batch in range(self.opt.input_nc):
            if not self.opt.rgb_in and self.opt.rgb_out:
                fake_B0_grad[self.id_[batch], batch, :,
                :] += torch.mean(real_A_grad[batch, :, :, :], 0) * 3
            else:
                # TODO  
                fake_B0_grad[batch,
                self.obs[batch] * np.array(self.opt.input_nc_1):(
                                                                            self.obs[
                                                                                batch] + 1) * np.array(
                    self.opt.input_nc_1), :, :] = real_A_grad[batch,
                                                  :, :, :]
    ...
    def get_current_visuals(self):
        ...
        fake_B_all = self.Tensor(real_B.size(0), real_B.size(1),
                                 ...
                                 fake_B.size(2) * fake_B.size(0))
        for b in range(g):
            real_A_all[:, :, :, b * m:m * (b + 1)] = real_A1[
                                                     b, :,
                                                     :, :]
            fake_B_all[:, :, :, b * m:m * (b + 1)] = fake_B[
                                                           b, :,
                                                           :, :]
        ...
        if not self.opt.no_Style2Glyph:
            if self.opt.conv3d:
                self.save_network(self.netG_3d, 'G_3d',
                                  self.gpu_ids, latest=latest, epoch=self.epoch_str1)
            self.save_network(self.netG, 'G', self.gpu_ids, latest=latest, epoch=self.epoch_str1)
        self.save_network(self.netE1, 'E1', self.gpu_ids, latest=latest, epoch=self.epoch_str1)
        self.save_network(self.netDE1, 'DE1', self.gpu_ids, latest=latest, epoch=self.epoch_str1)
        self.save_network(self.netD1, 'D1', self.gpu_ids, latest=latest, epoch=self.epoch_str1)
        if self.opt.which_model_preNet != 'none':
            self.save_network(self.preNet_A, 'PRE_A',
                              gpu_ids=self.gpu_ids, latest=latest, epoch=self.epoch_str1)
