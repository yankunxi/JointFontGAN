...
class sk1GANModel(BaseModel):
    ...
    def backward_G(self):
        ...
        self.loss_G_L1 = self.criterionL1(self.fake_B, self.real_B) \
                         * self.opt.lambda_A
        self.loss_G_MSE = self.criterionMSE(self.fake_B,
                                            self.real_B) \
                          * self.opt.lambda_A

        self.loss_G = self.criterionDiff0(self.fake_B,
                                          self.real_B) \
                      * self.opt.lambda_A
        ...
        if not self.skmode == 1:
            self.loss_Gsk0_L1 = self.criterionL1(self.fake_Bsk0,
                                                 self.real_Bsk0) \
                                * self.opt.lambda_A * 10
            self.loss_Gsk0_MSE = self.criterionMSE(self.fake_Bsk0,
                                                   self.real_Bsk0) \
                                 * self.opt.lambda_A * 10

            self.loss_Gsk0 = self.criterionDiff1(self.fake_Bsk0,
                                                 self.real_Bsk0) \
                             * self.opt.lambda_A * 10
            # self.loss_Gsk0 = self.loss_Gsk0_L1 * self.loss_Gsk0_MSE / 20
            self.loss_G += self.loss_Gsk0 + self.loss_Gsk0_GAN
        if not self.skmode == 0:
            self.loss_Gsk1_L1 = self.criterionL1(self.fake_Bsk1,
                                                 self.real_Bsk1) \
                                * self.opt.lambda_A * 10
            self.loss_Gsk1_MSE = self.criterionMSE(self.fake_Bsk1,
                                                   self.real_Bsk1) \
                                 * self.opt.lambda_A * 10

            self.loss_Gsk1 = self.criterionDiff1(self.fake_Bsk1,
                                                 self.real_Bsk1) \
                             * self.opt.lambda_A * 10
    ...
    def get_current_visuals(self):
        ...
        if self.output_str_indices:
            ...
            list = [('real_A', real_A), ('real_B', real_B),
                    ('fake_B', fake_B), ('fake_STR', img_output_str),
