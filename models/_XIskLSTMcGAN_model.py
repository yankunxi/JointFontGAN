...
class skLSTMcGANModel(BaseModel):
    def name(self):
        return 'skLSTMcGANModel'
    ...
    def initialize(self, opt):
        ...
        if self.opt.conv3d:
            self.netG_3d = XInetworks.define_G_3d(
                ...
                padding=3)
            self.netGcat_3d = XInetworks.define_G_3d(
                opt.input_nc * 2, opt.input_nc, norm=opt.norm,
                groups=opt.grps, gpu_ids=self.gpu_ids, ksize=7,
                padding=3)

        LSTM_hidden_size = 1536
        self.netG = XInetworks.define_LSTMG(opt.input_nc,
                                            opt.output_nc,
                                            opt.batchSize,
                                            opt.fineSize,
                                            self.remainnum,
        ...
        if not self.isTrain or (opt.which_epoch > 0):
            ...
            print("core load is " + str(sum(self.netG.LSTM_core.h)))
            if not self.skmode == 1:
    ...
    def set_test_input(self, input, time, time_):
        ...
        self.input_B.resize_(input_B.size()).copy_(input_B)
        if not self.skmode == 1:
            input_Ask0 = input['Ask0'][time % time_, :, :, :, :]
            input_Bsk0 = input['Bsk0'][time % time_, :, :, :, :]
            self.input_Ask0.resize_(input_Ask0.size()).copy_(input_Ask0)
            self.input_Bsk0.resize_(input_Bsk0.size()).copy_(input_Bsk0)
        if not self.skmode == 0:
            input_Ask1 = input['Ask1'][time % time_, :, :, :, :]
            input_Bsk1 = input['Bsk1'][time % time_, :, :, :, :]
            self.input_Ask1.resize_(input_Ask1.size()).copy_(input_Ask1)
            self.input_Bsk1.resize_(input_Bsk1.size()).copy_(input_Bsk1)
        self.image_paths = input['A_paths']
    ...
    def backward_D(self):
        ...
        if self.opt.conditional:
            ...
            if not self.skmode == 1:
                real_sk0 = torch.cat((self.real_Ask0,
            ...
            if not self.skmode == 0:
                real_sk1 = torch.cat((self.real_Ask1,
                                      self.real_B), 1).detach()
    ...
    def backward_G(self):
        ...
        if self.opt.conditional:
            ...
            if not self.skmode == 1:
                fake_sk0 = torch.cat((self.real_Ask0, self.fake_B), 1)
                ...
                if self.opt.which_model_preNet != 'none':
                    ...
                    self.loss_Gsk0_GAN += self.criterionGAN(
                        pred_fake_sk0, True)
            if not self.skmode == 0:
                fake_sk1 = torch.cat((self.real_Ask1, self.fake_B), 1)
        ...
        self.loss_G_L1 = self.criterionDiff0(self.fake_BB,
                                             self.real_BB) * \
                         self.opt.lambda_A
        self.loss_G = self.loss_G_GAN + self.loss_G_L1
        if not self.skmode == 1:
            self.loss_Gsk0_MSE = self.criterionDiff1(
                self.fake_Bsk0, self.real_Bsk0) * self.opt.lambda_A\
                                * 10
            self.loss_G += self.loss_Gsk0_MSE + self.loss_Gsk0_GAN
        if not self.skmode == 0:
            self.loss_Gsk1_MSE = self.criterionDiff1(
                self.fake_Bsk1, self.real_Bsk1) * self.opt.lambda_A\
                                * 10
            self.loss_G += self.loss_Gsk1_MSE + self.loss_Gsk1_GAN
    ...
    def get_current_visuals(self):
        ...
        if self.output_str_indices:
            ...
            list_ = [('Ereal_A', real_A), ('Efake_B', fake_B),
        ...
        if not self.skmode == 1:
            ...
            list_ += [('real_Ask0', real_Ask0), ('real_Bsk0',
                                                 real_Bsk0),
                     ('fake_Bsk0', fake_Bsk0)]
        if not self.skmode == 0:
            ...
            list_ += [('real_Ask1', real_Ask1), ('real_Bsk1',
                                                 real_Bsk1),
                     ('fake_Bsk1', fake_Bsk1)]
            # list += [('real_Ask1', real_Ask1),
            #          ('real_Bsk1', real_Bsk1)]
