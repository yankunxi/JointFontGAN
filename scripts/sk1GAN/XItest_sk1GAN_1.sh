...
EXPERIMENT_DIR="${DATASET}_${MODEL}_1"
...
--norm ${NORM} --str_input "${STR_INPUT}" --loadSize ${LOADSIZE} \
--fineSize ${FINESIZE} --display_id 0 --niter ${NITER} --niter_decay ${NITERD} \
--conditional --which_epoch ${EPOCH} --conv3d  --phase ${PHASE} --dspost_G ${DSPOSTG} \
--align_data --data_loader ${DATA_LOADER} --skmode ${SKMODE} #--stack_result
} 2>&1 | tee -a "${TESTLOG}"
