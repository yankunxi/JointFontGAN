...
experiment_dir="${DATASET}_cycleGAN_plus_64_small_train"
...
# =======================================
model_1_pretrained="./checkpoints_64cycle_small_${EPOCH}/GlyphNet_pretrain"
if [ ! -f "./checkpoints/${experiment_dir}/${EPOCH}_net_G.pth" ]; then
    cp "${model_1_pretrained}/${EPOCH}_net_G.pth" "./checkpoints/${experiment_dir}/"
    cp "${model_1_pretrained}/${EPOCH}_net_Gg.pth" "./checkpoints/${experiment_dir}/"
...
CUDA_VISIBLE_DEVICES=${CUDA_ID} python train_cycleGANplus.py --dataset ${DATASET}  --name ${NAME} --model ${MODEL}\
