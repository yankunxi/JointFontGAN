...
EXPERIMENT_DIR="${DATASET}_${MODEL}_blanks=0.85"
...
--data_loader ${DATA_LOADER} --skmode ${SKMODE} \
--conditional --downsampling_0_n ${DOWNSAMPLING_0_N} \
--downsampling_0_mult ${DOWNSAMPLING_0_MULT} --ngf ${NGF} --ndf ${NDF} --nif ${NIF} \
--str_output "${STR_OUTPUT}" --blanks ${BLANKS} #--zip_results 0 #--continue_latest # --checkpoints_dir "checkpoint"
} 2>&1 | tee -a "${TESTLOG}"
