...
--batchSize ${BATCHSIZE} --batchSplit ${BATCHSPLIT} --conditional --downsampling_0_n ${DOWNSAMPLING_0_N} \
--downsampling_0_mult ${DOWNSAMPLING_0_MULT} --ngf ${NGF} --ndf ${NDF} --nif ${NIF} --diff_loss0 ${DIFF_LOSS0} \
--save_epoch_freq 50 --print_freq 1 --skmode ${SKMODE} --data_loader ${DATA_LOADER} \
