...
experiment_dir="${DATASET}_MCGAN_train"
NAME="${experiment_dir}"
MODEL=StackGAN
...
for i in $all_epochs;
	...
	do CUDA_VISIBLE_DEVICES=${CUDA_ID} python test_Stack.py --dataset ${DATASET} --name "${experiment_dir}" --model ${MODEL}\
...
CUDA_VISIBLE_DEVICES=${CUDA_ID} python test_video.py --dataset ${DATASET} --name "${experiment_dir}" --model ${MODEL} \
								--which_model_netG ${MODEL_G} --which_model_netD ${MODEL_D} --n_layers_D ${n_layers_D} --norm ${NORM}\
