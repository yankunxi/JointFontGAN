...
EXPERIMENT_DIR="${DATASET}_${MODEL}2"
...
--save_epoch_freq 50 --display_freq 1 --print_freq 1 --skmode ${SKMODE} --data_loader ${DATA_LOADER} \
--conv3d --which_epoch ${LOAD_EPOCH} --diff_loss0 ${DIFF_LOSS0} --diff_loss1 ${DIFF_LOSS1} --stack_result
} 2>&1 | tee -a "${TRAINLOG}"
#{
#CUDA_VISIBLE_DEVICES=${CUDA_ID} python -u "${EXE_PATH}" --dataset "${DATASET}" --experiment_dir "${EXPERIMENT_DIR}" --model ${MODEL} \
#--which_model_netG ${MODEL_G} --which_model_netD ${MODEL_D}  --n_layers_D ${n_layers_D} --which_model_preNet ${PRENET} \
